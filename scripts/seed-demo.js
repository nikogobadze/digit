...
/* Task descriptions and review comments live in demo-content.json so the seed,
   the Georgian dictionary and the repair migration cannot drift apart. */
const CONTENT = require('./demo-content.json');
const DESC = Object.fromEntries(CONTENT.tasks.map(t => [t.en, t.d_en]));

/* Comments are chosen from the band that matches the stars. Picking from one
   flat pool put "Fixed in under an hour. Brilliant." on one-star reviews, which
   made the seeded reviews read as obvious nonsense. A couple of nulls in each
   band keep some ratings comment-free, as real ones are. */
const COMMENTS = {
  high: [...CONTENT.comments.high.map(c => c.en), null, null],   // 4-5 stars
  mid:  [...CONTENT.comments.mid.map(c => c.en), null],          // 3 stars
  low:  [...CONTENT.comments.low.map(c => c.en), null],          // 1-2 stars
};
const commentBand = (stars) => stars >= 4 ? COMMENTS.high : stars === 3 ? COMMENTS.mid : COMMENTS.low;
...
async function main() {
  ...
  for (let i = 0; i < TASK_COUNT; i++) {
    ...
    const ins = await db.execute({
      ...
      args: [client, cat, title, DESC[title] || title, urgency,
    ...
    if (chance(0.78)) {
      ...
      if (rated <= now) {
        ...
        const comment = pick(commentBand(stars));
